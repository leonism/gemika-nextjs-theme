      ...
      ...posts
        ...
        .map(post => ({
          ...
          relevance:
            ...
            ((post.frontmatter.tags as string[])?.some(tag => tag.toLowerCase().includes(query)) ? 1 : 0)
        })),
      ...projects
        ...
        .map(project => ({
          ...
          relevance:
            ...
            (project.frontmatter.excerpt?.toLowerCase().includes(query) ? 2 : 0) +
            ((project.frontmatter.tags as string[])?.some(tag => tag.toLowerCase().includes(query)) ? 1 : 0)
        }))
    ]
